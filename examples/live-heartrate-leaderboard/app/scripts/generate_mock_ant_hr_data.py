...
def load_mock_device_ids() -> list[int]:
    ...
    device_ids = [
        int(device_id)
        for device_id, data in user_db.items()
...
def generate_mock_ant_hr_data(ctx: TaskContext[RawAntHRPacket]) -> None:
    ...
    for device_id in device_ids:
        ...
        last_hr = (
            device_data[device_id]["hr_history"][-1]
            if device_data[device_id]["hr_history"]
            else None
        )
        ...
        if (
            device_data[device_id]["hr_event"]
            or device_data[device_id]["packet_number"] == 0
        ):
            device_data[device_id]["target_hr"] = generate_realistic_heart_rate(
                ...
                phase=device_data[device_id][
                    "phase"
                ],  # Phase can still be used for variation between devices
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Failed to send packet: {response.status_code}, {response.text}"
                )
    ...
    return {"task": "gen_stuff", "data": {"number_of_devices": len(device_ids)}}
...
def generate_ant_hrm_packet(device_id: int, device_data: dict):
    ...
    current_time = (
        device_data["packet_number"] * 0.25 * 1024
    )  # Convert to 1/1024 second resolution
    next_beat_time = (
        (60 * 1024 / device_data["target_hr"])
        + device_data["last_beat_time"]
        + (device_data["time_rollover"] * 65536)
    )
    ...
    data_payload = [
        ...
        int(device_data["previous_beat_time"])
        & 0xFF,  # Byte 2: Previous Heart Beat Event Time LSB
        (int(device_data["previous_beat_time"]) >> 8)
        & 0xFF,  # Byte 3: Previous Heart Beat Event Time MSB
        int(device_data["last_beat_time"]) & 0xFF,  # Byte 4: Heart Beat Event Time LSB
        (int(device_data["last_beat_time"]) >> 8)
        & 0xFF,  # Byte 5: Heart Beat Event Time MSB
        device_data["beat_count"],  # Byte 6: Heart Beat Count
        device_data[
            "target_hr"
        ],  # Byte 7: Computed Heart Rate (use target HR if no history)
    ]
...
def generate_realistic_heart_rate(
    time_elapsed, base_hr=60, max_hr=180, last_hr=None, phase=0.5
):
    ...
    if current_interval < high_intensity_duration:
        ...
        target_hr = low_hr + (high_hr - low_hr) * (
            current_interval / high_intensity_duration
        )
    else:
        # Ramp down during recovery phase (simplified)
        target_hr = high_hr + (low_hr - high_hr) * (
            (current_interval - high_intensity_duration)
            / (interval_duration - high_intensity_duration)
        )
    ...
    return int(
        max(base_hr * 0.9, min(round(new_hr), max_hr))
    )  # Allow slightly below base_hr


ingest_task = Task[RawAntHRPacket, None](
    name="task", config=TaskConfig(run=generate_mock_ant_hr_data)
)
